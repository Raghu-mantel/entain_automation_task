...
export const usePollRaces = (
  filters: RacingCategoryFilters,
): { races: ListRace[]; isLoading: boolean } => {
  ...
  const filterRaces = (races: ListRace[]): ListRace[] => {
    ...
    return races
      .filter((race) => {
        return (
          filters[race.categoryId] &&
          race.advertisedStart.getTime() - currentTime > -300_000
        );
      })
      .slice(0, 5);
  };
  ...
  const sortRaces = (races: ListRace[]): ListRace[] =>
    races.sort(
      (a, b) => a.advertisedStart.getTime() - b.advertisedStart.getTime(),
    );
  ...
  const fetchRaces = async (): Promise<void> => {
    const encodedCategoryIds = encodeURIComponent(
      JSON.stringify(RACING_CATEGORIES),
    );
    ...
    ).json()) as NextRacesCategoryGroupResponse;

    const listRaces = Object.values(response.race_summaries).map<ListRace>(
      (rawRace) => ({
